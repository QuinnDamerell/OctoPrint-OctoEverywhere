...
class TestNotificationsHandler(unittest.TestCase):
    ...
    def test_notification_methods_include_platform_error_info(self) -> None:
        ...
        handler.OnPaused("file.gcode", platformErrorCode=2502, platformErrorMessage="Paused by printer")
        handler.OnError("Printer error", platformErrorCode="machine_status=14", platformErrorMessage="Emergency Stop")
        handler.OnFailed("file.gcode", None, "cancelled", platformErrorCode="STOPPED", platformErrorMessage="Stopped")
        handler.OnFilamentChange(platformErrorCode="07008011", platformErrorMessage="Filament run out")
        handler._clearSpammyEventContexts()
        handler.OnUserInteractionNeeded(platformErrorCode="action:paused", platformErrorMessage="Paused for user")

        self.assertEqual(sent[0], ("paused", {
            "PlatformErrorCode": "2502",
            "PlatformErrorMessage": "Paused by printer",
        }))
        self.assertEqual(sent[1], ("error", {
            "Error": "Printer error",
            "PlatformErrorCode": "machine_status=14",
            "PlatformErrorMessage": "Emergency Stop",
        }))
        self.assertEqual(sent[2], ("failed", {
            "Reason": "cancelled",
            "PlatformErrorCode": "STOPPED",
            "PlatformErrorMessage": "Stopped",
        }))
        self.assertEqual(sent[3], ("filamentchange", {
            "PlatformErrorCode": "07008011",
            "PlatformErrorMessage": "Filament run out",
        }))
        self.assertEqual(sent[4], ("userinteractionneeded", {
            "PlatformErrorCode": "action:paused",
            "PlatformErrorMessage": "Paused for user",
        }))


    def test_common_event_args_preserve_platform_error_info_in_rest_body(self) -> None:
        handler = self._MakeHandler()
        ...
        self.assertEqual(args["PlatformErrorMessage"], "Filament run out")
