...
from octoeverywhere.interfaces import IWebRequestHandler
...
class MoonrakerWebRequestResponseHandler(IWebRequestHandler):
    ...
    def HandleResponse(self, contextObject:ResponseHandlerContext, octoHttpResult:HttpResult, bodyBuffer:Buffer) -> Buffer:
        try:
            ...
            Sentry.OnException("MainsailConfigHandler exception while handling mainsail config.", e)
        return bodyBuffer


    def _HandleMainsailConfig(self, octoHttpResult:HttpResult, bodyBuffer:Buffer) -> Buffer:
        #
        ...
        try:
            mainsailConfig = json.loads(bodyBuffer.GetBytesLike().decode("utf8"))
            ...
            return Buffer(json.dumps(mainsailConfig, indent=4).encode("utf8"))
        except Exception as e:
            body = None
            try:
                body = bodyBuffer.GetBytesLike().decode("utf8")
            except Exception:
                pass
            Sentry.OnException(f"MainsailConfigHandler exception while handling mainsail config. {octoHttpResult.StatusCode}; body: {body}", e)
        # On failure, return the original result.
        return bodyBuffer



    def _HandleWebRtcSdpResponse(self, octoHttpResult:HttpResult, bodyBuffer:Buffer) -> Buffer:
        #
