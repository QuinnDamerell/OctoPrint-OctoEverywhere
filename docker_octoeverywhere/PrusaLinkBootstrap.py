...
class PrusaLinkBootstrap:
    ...
    def Bootstrap(logger:logging.Logger, config:Config) -> None:
        ...
        if printerIp is None:
            ...
            logger.error("")
            logger.error("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
            logger.error("                    You must provide your PrusaLink's IP address.")
        ...
        printerPort = os.environ.get("PORT", None)
        if printerPort is not None:
            logger.info(f"Setting PrusaLink Port: {printerPort}")
        ...
        if printerPort is None:
            ...
            logger.info(f"Setting The Default PrusaLink Port: {printerPort}")
            config.SetStr(Config.SectionCompanion, Config.CompanionKeyPort, printerPort)
        try:
            ...
            logger.error("")
            logger.error("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
            logger.error("                 The PrusaLink port must be a number between 1 and 65535.")
            logger.error("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
            logger.error("")
            logger.error("")
            time.sleep(5.0)
            sys.exit(1)
        logger.info(f"Target PrusaLink Port: {printerPort}")
        ...
        if apiKey is not None:
            logger.info("Setting PrusaLink API key auth.")
        ...
        if username is not None:
            logger.info(f"Setting PrusaLink Username: {username}")
        ...
        if password is not None:
            logger.info("Setting PrusaLink Password.")
        ...
        if apiKey is None and password is None and username is None:
            ...
            logger.error("")
            logger.error("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
            logger.error("              You must provide your PrusaLink username/password or API key.")
        ...
        if apiKey is not None:
            logger.info("Using PrusaLink API key auth.")
        ...
        logger.info("Setting PrusaLink username/password auth.")
